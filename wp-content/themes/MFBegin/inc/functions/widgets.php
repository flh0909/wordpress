...
<div class="new_cat">
	<ul>
		<?php query_posts( array ( 'showposts' => $number, 'orderby' => date, 'caller_get_posts' => 1 ) ); while ( have_posts() ) : the_post(); ?>
		<li>
		...
		foreach ($comments as $my_comment) {
			if ($my_comment->comment_author_email != $my_email) {
				...
				<li>					
					...
					<a href="<?php echo get_permalink($my_comment->comment_post_ID); ?>#comment-<?php echo $my_comment->comment_ID; ?>" title="<?php echo $my_comment->comment_author; ?>在《<?php echo get_the_title($hot_comment->comment_post_ID); ?>》发表的评论" >
