...
<h2>又拍云插件设置</h2>
	<form name="upyun_form" method="post" action="<?php echo admin_url('options-general.php?page=set_upyun_option'); ?>">
		<table class="form-table">
			...
			<tr valign="top">
				...
				<td><input name="bucket_name" type="text" class="regular-text" size="100" id="rest_server" value="<?php echo $this->option['bucket_name']; ?>" /> <span class="description">创建的空间名称</span></td>
			</tr>
			<tr valign="top">
			...
			<tr valign="top">
				...
				<td><input name="remote_upload_root" placeholder="例如：/wp-file" type="text" class="regular-text" size="100" id="rest_server" value="<?php echo $this->option['remote_upload_root']; ?>" /> <span class="description">文件存放在又拍云空间的根目录，默认根目录  "/"，例如："/wp-file"，文件将存放在此目录中</span></td>
			</tr>
			<tr valign="top">
				<th scope="row">创建的空间类型:</th>
				<td>
					<p>
					...
					<p class="description">因为又拍云的空间类型（普通、图片）不同，如果选择是图片空间，非图片格式的文件将不会上传到又拍云</p>
				</td>
			</tr>
			<tr valign="top">
	...
	<p>PS2: 如果又拍云空间类型是图片空间，非图片格式的附件将不计在内</p>
	<p>PS3: 请确保附件都上传完毕后（也可以通过FTP工具），再启用又拍云服务</p>
